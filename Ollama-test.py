...
# 初始化 Ollama 客户端
ollama_llm1 = ChatOllama(model="deepseek-r1:1.5b")   # 1.5b
ollama_llm2 = ChatOllama(model="deepseek-r1:latest") # 7b
...
def generate_summary(news_text: str) -> str:
    try:
        ...
        response = ollama_llm1.invoke(messages)
...
# --- 定义评价摘要质量的函数 ---
def evaluate_summary(news_text: str, summary: str) -> str:
    try:
        prompt = (
            ...
            "- 评分必须在0～10之间；\n"
            "- 评分结果可以是整数或保留一位小数；\n"
            "- 返回的内容**仅**包含评分, 包含五个维度的得分和最终评分，例如：\n"
            "  '准确性：9分，完整性：8分，简洁性：9分，语言流畅度：9分，逻辑清晰度：8分，最终评分：8.6分'。\n\n"
            f"新闻原文：{news_text}\n\n摘要：{summary}\n\n"
            "请严格按照上述要求返回最终评分。"
        )
        messages = [
        ...
        response = ollama_llm2.invoke(messages)
        evaluation = str(response)
        return evaluation
    except Exception as e:
        logger.error(f"摘要评价失败: {e}")
        return ""
...
if __name__ == "__main__":
    results = evaluate_news_summaries("data-test.txt")
    print("评估完成，结果已保存")
